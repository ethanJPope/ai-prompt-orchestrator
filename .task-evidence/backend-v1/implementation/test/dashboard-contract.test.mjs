...
test("the Observatory exposes live-run UI and event-stream wiring", async () => {
  ...
  assert.match(server, /api\/account\/activate/);
  assert.match(server, /api\/account\/deactivate/);
  assert.match(server, /api\/customer\/stats/);
  assert.match(server, /api\/business\/stats/);
  assert.match(controller, /correlationId/);
  assert.match(controller, /reviewerStatuses/);
  assert.match(controller, /waveHandoffs/);
  assert.match(controller, /finalArbiterResult/);
});
