...
8. Run every returned reviewer assignment using `executionPolicy.reviewWaves`: dispatch each parallel wave concurrently up to the available agent limit, then continue to the next wave. Use `collaboration` as the shared message protocol: the primary agent brokers append-only reviewer messages, each reviewer reads prior findings, and each output must confirm, contradict, or extend prior evidence when applicable. Reviewers are read-only, honor their individual budgets, and return no more than three evidence-backed findings. Record completion or timeout; never add an unplanned replacement reviewer.
